...
export default function CarritoPage() {
  const { carrito, removeFromCart, clearCart, total } = useCart();
  const { user } = useAuth();
            ...
            <div className="flex-1 space-y-4">
              ...
              {carrito.map((item) => (
                <div key={item.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 flex gap-4 items-center">
                  {item.imagen_url ? (
                    <img src={item.imagen_url} alt={item.nombre} className="w-20 h-20 object-cover rounded-xl" />
                  ) : (
                  ...
                  <div className="flex-1">
                    ...
                    {tieneOferta(item) ? (
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <p className="text-red-600 font-extrabold text-lg">S/. {(item.precio_final * item.cantidad).toFixed(2)}</p>
                      </div>
                    ) : (
                      <p className="text-blue-600 font-extrabold text-lg mt-1">S/. {(item.precio_final * item.cantidad).toFixed(2)}</p>
                    )}
                  </div>
